...
<main class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div id="cart-items-container" class="space-y-4">
            @forelse ($cartItems as $index => $item)
                <div class="flex justify-between items-center bg-white text-[#B97D0E] rounded-lg px-6 py-3 shadow cart-item"
                     data-index="{{ $index }}" data-item-price="{{ $item['item_price'] }}">
                    {{-- Product --}}
                    ...
                    <div class="flex items-center gap-2 bg-gray-100 px-4 py-1 rounded-full">
                        <button class="quantity-minus text-lg font-bold">-</button>
                        <span class="quantity-display border-x px-3">{{ $item['quantity'] }}</span>
                        <button class="quantity-plus text-lg font-bold">+</button>
                    </div>
            ...
            @empty
                <div class="bg-white text-center py-10 rounded shadow">
                    ...
                    <a href="{{ route('all') }}" class="mt-4 inline-block bg-orange-500 text-white px-6 py-2 rounded-full hover:bg-orange-600">Start Shopping</a>
                </div>
            @endforelse
        </div>
    </section>
    ...
    <aside class="bg-white rounded-lg px-6 py-6 shadow-md space-y-4 h-fit">
        ...
        <button class="w-full bg-yellow-700 text-white font-semibold py-2 rounded-full disabled:opacity-50"
